...
such as readability. Generally speaking, software developers should ideally value keeping their
code within defined standards. The code style can be viewed as an aesthetic issue directly related
to the taste of the developer. However, there are some styles adopted that can benefit both the 
developer and others who may use the code developed. 
...
example, when the variable keeps its original value, it tells the user that it can use const instead of let. Another
...
### GOOD
<img class="ui medium floated image" src="../images/codestyle1.png">
*Code from in-class WOD. "Five problems every software engineer should be able to solve (Part 3)", ICS 314*

### BAD
<img class="ui medium floated image" src="../images/codestyle12.png">

We see that the same code is much more readable, clean, and aesthetically more beautiful than the one from image 2.
Even if you program on your own, you should adopt this good practice, and if you share the source with other people,
it is mandatory, so others do not spend unnecessary time trying to understand the meaning of the code.

## Conclusion
Those small rules can make the code more readable, and life easier for us. You will have a hard time finding errors or
maintaining them if you do not code in a simple, clear, organized way and writing comments. Of course, not all programming
languages are the same, but there are many good practices that can be followed in the same language. When we follow the 
coding style, we guarantee that everyone will have a minimal understanding of the code being developed, even someone with
the least knowledge of syntax. In view of this, it is recommended to always adopt the code style of the language being used.
